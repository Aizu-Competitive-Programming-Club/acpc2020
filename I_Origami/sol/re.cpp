...
V compress(V vs){
  ...
  auto cmp=[&](auto x,auto y){return abs(x-y)<EPS;};
  vs.erase(unique(vs.begin(),vs.end(),cmp),vs.end());
  return vs;
}
...
signed main(){
  ...
  auto mirrored=[&](Polygon ps,Line l){
    Polygon qs;
    for(auto p:ps)
      qs.emplace_back(reflect(l,p));
    return andrewScan(qs);
  };
  ...
  for(int i=0;i<m;i++){
    ...
    vector<Polygon> vq;
    for(auto ps:vp){
      auto ls=convexCut(ps,Line(p,q));
      auto rs=convexCut(ps,Line(q,p));
      vq.emplace_back(mirrored(ls,Line(p,q)));
      vq.emplace_back(rs);
    }
    swap(vp,vq);
  }
  ...
  for(auto ps:vp){
    for(auto qs:vp){
      int x=ps.size(),y=qs.size();
      for(int i=0;i<x;i++){
        for(int j=0;j<y;j++){
          Segment u(ps[i],ps[(i+1)%x]);
          Segment v(qs[j],qs[(j+1)%y]);
          if(getDistanceSS(u,v)==0)
            xs.emplace_back(getCrossPointLL(u,v).x);
        }
      }
    }
  }
  ...
  for(int t=0;t+1<(int)xs.size();t++){
    ...
    for(auto ps:vp){
      ...
      for(int i=0;i<x;i++){
        Segment u(ps[i],ps[(i+1)%x]);
        if(!(min(u.p1.x,u.p2.x)<=xs[t+0]+EPS)) continue;
        if(!(max(u.p1.x,u.p2.x)>=xs[t+1]-EPS)) continue;
        vs.emplace_back();
    ...
    auto nearest=[&](double x)->double{
      ...
      if(it==sx.begin()) return *it;
      auto pr=it;
      pr--;
      if(it==sx.end()) return *pr;
      return abs(*pr-x)<abs(*it-x)?*pr:*it;
    };
    ...
    for(int i=0;i+1<(int)vs.size();i++){
      Polygon ps;
      ps.emplace_back(xs[t+0],vs[i+0].first);
      ps.emplace_back(xs[t+0],vs[i+1].first);
      ps.emplace_back(xs[t+1],vs[i+0].second);
      ps.emplace_back(xs[t+1],vs[i+1].second);

      assert(vs[i+0].second<=vs[i+1].second+EPS);

      if(abs(vs[i+0].first-vs[i+1].first)<EPS and
