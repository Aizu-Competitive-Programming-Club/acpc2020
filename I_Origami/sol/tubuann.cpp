...
namespace Geometry{
  ...
  //typedef complex<long double> P;
    ...
    static bool comp(const P &p1,const P &p2){return p1.real()==p2.real()?p1.imag()<p2.imag():p1.real()<p2.real();}
    ...
    D area(const vector<P> &poly){
        D ans=0;
        for(int i=2;i<(int)poly.size();i++){ans+=cross(poly[i-1]-poly[0],poly[i]-poly[0]);}
        return abs(ans)/2;
    }
    ...
    D closestpair(vector<P> pt){
        ...
        D ret=INF;
        for(ll i=1;i<(int)pt.size();i<<=1){
          for(ll j=0;i+j<(int)pt.size();j+=i*2){
                ll m=i+j;
                vector<P> R;
                D l=-INF,r=INF;
                for(ll k=j;k<m;k++){l=max(l,pt[k].real());}
                for(ll k=0;m+k<(int)pt.size() && k<i;k++){r=min(r,pt[m+k].real());}
                for(ll k=0;m+k<(int)pt.size() && k<i;k++){if(pt[m+k].real()-l<ret){R.push_back(pt[m+k]);}}
                ll idx=0;
                for(ll k=j;k<m;k++){
                    if(r-pt[k].real()>ret){continue;}
                    while(idx<(int)R.size() && pt[k].imag()-R[idx].imag()>ret){idx++;}
                    for(ll n=idx;n<(int)R.size() && R[n].imag()-pt[k].imag()<ret;n++){ret=min(ret,abs(R[n]-pt[k]));}
                }
                inplace_merge(pt.begin()+j,pt.begin()+m,j+i*2<(int)pt.size()?pt.begin()+j+2*i:pt.end(),comp_y);
            }
        }
        return ret;
    }
...
using convex=pair<vector<P>,int>;
using vec=vector<P>;

vector<convex> now,rg,lf,nx;

convex mul(const convex &A,const convex &B){
  auto U=A.F;
  auto &V=B.F;
  int sz=V.size();
  for(int i=0;i<sz && area(U)>EPS;i++){
    U=convexcut(U,V[i],V[(i+1)%sz]);
  }
  return {U,A.S+B.S};
}

vector<convex> rem(const convex &A,const convex &B){
  vec X=A.F;
  vector<convex> ret;
  auto &V=B.F;
  int sz=V.size();
  for(int i=0;i<sz && area(X)>EPS;i++){
    auto ps=convexcut(X,V[(i+1)%sz],V[i]);
    if(area(ps)>EPS){ret.push_back({ps,A.S});}
    X=convexcut(X,V[i],V[(i+1)%sz]);
  }
  return ret;
}

void other_cul(const vector<convex> &A,const vector<convex> &B){
  vector<convex> a1=A,a2;
  for(auto &V:B){
    a2.clear();
    for(auto &U:a1){
      for(auto x:rem(U,V)){a2.push_back(x);}
    }
    swap(a1,a2);
  }
  for(auto &x:a1){nx.push_back(x);}
}
...
void cross_cul(){
  for(auto &U:rg){
    for(auto &V:lf){
      auto ps=mul(U,V);
      if(area(ps.F)>EPS){nx.push_back(ps);}
    }
  }
}
...
void cut(P a,P b){
  for(auto &U:now){
    auto A=convexcut(U.F,b,a);
    if(area(A)>EPS){rg.push_back({A,U.S});}
    A=convexcut(U.F,a,b);
    if(area(A)>EPS){lf.push_back({A,U.S});}
  }
  for(auto &U:lf){
    for(auto &p:U.F){
      p=reflect(b,a,p);
    }
    reverse(U.F.begin(),U.F.end());
  }
}

void culc(P a,P b){
  rg.clear(); lf.clear(); nx.clear();
  cut(a,b);
  cross_cul();
  other_cul(rg,lf);
  other_cul(lf,rg);
