...
$M$ 回の操作後、各 $i = 1, 2, 3, \dots, 2^M$ に対して、ちょうど $i$ 枚の紙が重なっている部分の面積を求めよ。
...
## @{keyword.output}
$2^M$ 行出力する。 $i$ 行目には ちょうど $i$ 枚の紙が重なっている部分の面積を出力せよ。また出力値の絶対誤差または相対誤差が $10^{-@{param.significant_digit}}$ 以下の場合、正答と判定される。
