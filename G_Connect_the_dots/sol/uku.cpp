...
signed main(){
  ...
  const int m = 3 * n;
  RangeToRange R(m);
  Dijkstra<int> D(m*3+2*n);

  auto f=[&](int u,int v){D.add_edge(u,v,0);};
  R.init(f);
  ...
  for(int k=1;k<=2;k++){
    ...
    // connect
    for(int i=0;i<n;i++){
      D.add_edge(m+i,m+k*n+dx[xs[i]],1);
    ...
    // add point
    for(int i=0;i<n;i++)
      R.add_edge(k*n+dx[ls[ys[i]]],k*n+dx[rs[ys[i]]]+1,
                 k*n+dx[ls[ys[i]]],k*n+dx[rs[ys[i]]]+1,f,f);
